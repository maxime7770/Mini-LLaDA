...
def load_refinedweb_dataset(path_pattern="data_finetune/train-*.parquet"):
    files = glob.glob(path_pattern)
    samples = []
    for file in files:
        df = pl.read_parquet(file)
        for row in df["messages"].to_list():
            ...
            for msg in row:
                ...
                if role == "user":
                    ...
                    if conversation_history:  # Only create a sample if there's conversation history
                        prompt = "\n".join(conversation_history)
                        response = "Assistant: " + text
                        sample_text = prompt + " " + "<sep>" + " " + response
                        samples.append(sample_text)
                    # Add the assistant reply to the history so subsequent turns include it.
                    conversation_history.append("Assistant: " + text)
                    conversation_history = []
                else:
                    # If there's an unexpected role, just append the text.
                    conversation_history.append(text)
    print(f"Generated {len(samples)} finetuning samples.")
    return random.sample(samples, min(ROWS, len(samples)))
...
def get_vocab():
    ...
    return vocab, hf_tokenizer.pad_token_id, hf_tokenizer.mask_token_id, hf_tokenizer.sep_token_id
